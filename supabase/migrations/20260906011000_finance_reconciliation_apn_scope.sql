-- Finance v5 reconciliation: only require a Finance commission mapping when the
-- APN ledger event still has a corresponding posted APN income/project. Historical
-- orphaned ledger rows (for deleted/unposted collections) are not counted as a
-- Finance exception because there is no company cash transaction to reconcile.
begin;
