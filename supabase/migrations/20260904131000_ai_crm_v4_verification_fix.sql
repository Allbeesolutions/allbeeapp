...
begin
 if not public.is_admin() and auth.uid() is null then raise exception 'Authentication required.' using errcode='not_authenticated'; end if;
 select count(*) into v_total from public.ai_crm_actions;
 select count(*) into v_accepted from public.ai_crm_actions where delivery_state='accepted';
