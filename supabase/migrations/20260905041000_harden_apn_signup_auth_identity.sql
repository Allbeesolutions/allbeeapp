-- Harden the APN signup guard after the State Head guard replacement.
...
begin
  ...
  if tg_op = 'INSERT' then
    ...
    if not public.is_admin() and not signup_bootstrap and (auth.uid() is null or (new.data->>'id') <> auth.uid()::text) then
