...
begin
  ...
  where s.earning_id = e.id and (e.project_id = p_project_id or e.source_collection_id in (select id from public.apn_revenue_collections where project_id = p_project_id));
  delete from public.apn_referral_earnings e
  where e.project_id = p_project_id or e.source_collection_id in (select id from public.apn_revenue_collections where project_id = p_project_id);
  ...
  delete from public.apn_timeline where data->>'relatedId' = p_project_id or id like '%:commission-project:' || p_project_id or id like '%:commission-project-completed:' || p_project_id;
  ...
  insert into public.audit(id, data, updated_at) values (audit_id, jsonb_build_object('id', audit_id, 'ts', (extract(epoch from now()) * 1000)::bigint, 'user', coalesce(public.current_name(), 'Super Admin'), 'userId', auth.uid()::text, 'action', 'purged cancelled APN commission project', 'module', 'APN', 'entity', 'APN Commission Project', 'entityId', p_project_id, 'metadata', jsonb_build_object('projectName', p.project_name, 'clientName', p.client_name, 'partnerId', p.partner_id, 'projectValue', p.project_value, 'reason', 'User-requested removal of test commission project')), now());
  return jsonb_build_object('deleted', deleted_project = 1, 'projectId', p_project_id, 'collectionsDeleted', deleted_collections, 'transactionsDeleted', deleted_transactions, 'timelineDeleted', deleted_timeline, 'notificationsDeleted', deleted_notifications);
end;
$$;
