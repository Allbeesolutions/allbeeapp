-- Fix the finance sync trigger's transactions shape and recursion guard,
-- then restore the private engine finance bridge.
begin;

-- transactions.kind is stored inside JSONB data, not as a table column.
-- The trigger also updates transactions, so nested execution must stop.
do $$
declare v_sql text;
begin
...
-- body remains unchanged; only the direct-caller privilege gate is removed.
do $$
declare v_sql text;
begin
