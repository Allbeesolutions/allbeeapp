-- APN Team Chat production schema-cache repair.
-- The parameterized person-conversation RPC exists in migrations, but PostgREST
-- can retain a stale zero-argument signature in its schema cache after a rapid
-- sequence of CREATE OR REPLACE patches. Re-assert the exact grant and force a
-- PostgREST schema reload without changing conversation/message data.
...
begin
  ...
  select u.id, u.data->>'name', u.data->>'apnId'
  ...
  insert into public.apn_chat_conversations
    (id, type, slug, subject, created_by, created_at, updated_at)
  values
    (gen_random_uuid(), 'person', v_slug, v_other_name, v_self, now(), now())
...
-- Force PostgREST to rebuild its RPC schema cache immediately.
