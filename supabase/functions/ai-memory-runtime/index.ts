...
Deno.serve(async(req)=>{
 ...
 try{
  ...
  if(workerKey&&secret&&workerKey===secret) client=admin; else {if(!auth)return json({error:"Authentication required."},401);const {data:u,error:ue}=await client.auth.getUser();if(ue||!u.user)return json({error:"Authentication required."},401);}
  ...
  if(mode==="index"){const {error:syncError}=await client.rpc("ai_memory_sync_knowledge");if(syncError&&auth)return json({error:syncError.message},400);if(auth){const {error:businessSyncError}=await client.rpc("ai_memory_sync_business");if(businessSyncError)return json({error:businessSyncError.message},400);}const {data:docs,error:readError}=await admin.from("ai_memory_documents").select("id,title,content,metadata,content_hash").eq("active",true).is("embedding",null).limit(100);if(readError)return json({error:readError.message},500);if(!docs?.length)return json({ok:true,indexed:0,pending:0});const vectors=await embed(key,docs.map(d=>`${d.title}\n${d.content}`));let indexed=0;for(let i=0;i<docs.length;i++){const {error}=await admin.from("ai_memory_documents").update({embedding:vectors[i]}).eq("id",docs[i].id).eq("content_hash",docs[i].content_hash);if(!error)indexed++;}const {count}=await admin.from("ai_memory_documents").select("id",{count:"exact",head:true}).eq("active",true).is("embedding",null);return json({ok:true,indexed,pending:count||0});}
