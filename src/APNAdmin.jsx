import { APNAdminHub, APNAdminPartners, APNAdminLeads, APNCommissionReverseModal, APNTagForm, APNPartnerDocumentForm, APNCommunicationForm } from "./APNPartnerProfile.jsx";
...
const LazyAPNPartnerProfile = React.lazy(() => import("./APNPartnerProfile.jsx"));
...
export default function APNAdmin(props) {
  ...
  return (
    <div className="content">
      ...
      {modal?.type === "apnWarning" && <APNWarningForm runtime={props.runtime} partner={modal.partner} onClose={() => setModal(null)} onSave={(value) => saveWarning(modal.partner, value)} />}
      ...
      {modal?.type === "apnTags" && <APNTagForm runtime={props.runtime} partner={modal.partner} onClose={() => setModal(null)} onSave={(tags) => saveTags(modal.partner, tags)} />}
      {modal?.type === "apnDocument" && <APNPartnerDocumentForm runtime={props.runtime} partner={modal.partner} initial={modal.initial} onClose={() => setModal(null)} onSave={(value) => savePartnerDocument(modal.partner, value, modal.initial)} />}
      {modal?.type === "apnCommunication" && <APNCommunicationForm runtime={props.runtime} partner={modal.partner} onClose={() => setModal(null)} onSave={(value) => saveCommunication(modal.partner, value)} />}
      ...
      {modal?.type === "apnCommissionReverse" && <APNCommissionReverseModal runtime={props.runtime} commission={modal.commission} partnerName={modal.partnerName} isSuper={modal.isSuper} onClose={() => setModal(null)} onSave={(reason, unlockPaid) => reverseCommissionNow(modal.commission, reason, unlockPaid)} />}
