...
export function printProposalDocument(detail, runtime) {
  ...
  if (!w) return;
  const body = sections.filter((s) => s.enabled !== false).sort((a, b) => (a.sort_order || 0) - (b.sort_order || 0)).map((s) => `<section><h2>${escapeProposalHtml(s.name)}</h2><div class="section-body">${escapeProposalHtml(proposalSectionDisplay(s, runtime.money)).replace(/\n/g, "<br>")}</div></section>`).join("");
