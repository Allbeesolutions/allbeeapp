...
export default function AIIntelligenceCenter(props) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      try { const { data: queueRows } = await sb.from("business_automation_queue").select("id,rule_id,entity,entity_id,status,payload,requested_at").eq("status", "pending_approval").order("requested_at", { ascending: false }).limit(20); setAutomationQueue(Array.isArray(queueRows) ? queueRows : []); } catch { setAutomationQueue([]); }
      try { const { data: health } = await sb.rpc("app_error_health"); setErrorHealth(health || null); } catch { setErrorHealth(null); }
      try {
  ...
  const reviewAutomation = async (id, approve) => {
    setAutomationBusy(true); setError("");
    try { const fn = approve ? "business_automation_approve" : "business_automation_reject"; const { error: e } = await sb.rpc(fn, { p_id: id, ...(approve ? {} : { p_reason: "Human reviewer rejected this automation proposal." }) }); if (e) throw new Error(e.message); await load(); emitToast(approve ? "Automation approved." : "Automation rejected.", "success"); }
    catch (e) { setError(e.message || "Automation review failed."); } finally { setAutomationBusy(false); }
  };
  ...
  const executiveSummary = snapshot?.executive_summary || "";
  ...
  return (
    <div className="content">
      ...
      {!snapshot ? <div className="cards-grid"><div className="card" aria-busy="true"><div className="skeleton skeleton-line" style={{ width: "38%" }} /><div className="skeleton" style={{ height: 90, marginTop: 12 }} /></div><div className="card" aria-busy="true"><div className="skeleton skeleton-line" style={{ width: "55%" }} /><div className="skeleton" style={{ height: 90, marginTop: 12 }} /></div></div> : <>
        ...
        {tab === "automation" && <div className="cards-grid" style={{ gridTemplateColumns: "repeat(auto-fit,minmax(300px,1fr))", alignItems: "start" }}><div className="card"><div className="item-row"><div className="item-main"><div className="item-title">Business Automation Engine</div><div className="item-meta">Safe, deterministic review automations. They create notifications only; they never mutate CRM, finance, APN or customer records.</div></div><Sparkles size={16} color="var(--primary)" /></div>{automationRules.map((r) => <div className="item-row" key={r.id}><div className="item-main"><div className="item-title">{r.title}</div><div className="item-meta">{r.condition}</div></div><span className={`badge ${r.level === "Urgent" ? "neg" : "accent"}`}>{r.level}</span></div>)}<button className="btn primary" onClick={runAutomation} disabled={automationBusy}><RefreshCw size={14} />{automationBusy ? "Running…" : "Run review automations"}</button></div><div className="card"><div className="item-row"><div className="item-main"><div className="item-title">Approval queue</div><div className="item-meta" style={{ marginTop: 6, lineHeight: 1.6 }}>Scheduled and event-driven detections are queued first. Nothing is executed by the automation engine without explicit human approval.</div></div><span className="badge pri">{automationQueue.length} pending</span></div>{automationQueue.length ? automationQueue.map((x) => <div className="item-row" key={x.id}><div className="item-main"><div className="item-title">{x.payload?.title || x.rule_id}</div><div className="item-meta">{x.payload?.customer_name || x.payload?.quote_number || x.payload?.reason || x.entity}</div><div className="hint-line">Requested {new Date(x.requested_at).toLocaleString()}</div></div><div className="row-actions"><button className="btn sm primary" disabled={automationBusy} onClick={() => reviewAutomation(x.id, true)}>Approve</button><button className="btn sm" disabled={automationBusy} onClick={() => reviewAutomation(x.id, false)}>Reject</button></div></div>) : <Empty icon={<CheckCircle2 size={22} />} title="No pending automation approvals" text="The hourly scheduler and event triggers have no new proposals awaiting review." />}<div className="calc-box" style={{ marginTop: 14 }}><div className="calc-row"><span>Active rules</span><b className="mono">{automationRules.length}</b></div><div className="calc-row"><span>Auto-mutations</span><b className="mono">0</b></div></div></div></div>}
        {tab === "observability" && <div className="cards-grid" style={{ gridTemplateColumns: "repeat(auto-fit,minmax(280px,1fr))", alignItems: "start" }}><div className="card"><div className="item-title">Runtime health</div><div className="item-meta" style={{ marginTop: 6 }}>Grouped client errors from the production error telemetry stream.</div><div className="calc-box" style={{ marginTop: 12 }}><div className="calc-row"><span>Last 24 hours</span><b className="mono">{errorHealth?.last_24h ?? "—"}</b></div><div className="calc-row"><span>Unresolved</span><b className="mono">{errorHealth?.unresolved ?? "—"}</b></div><div className="calc-row"><span>Critical unresolved</span><b className="mono">{errorHealth?.critical ?? "—"}</b></div></div></div><div className="card"><div className="item-title">Error groups</div>{(errorHealth?.groups || []).length ? errorHealth.groups.map((g) => <div className="item-row" key={g.fingerprint}><div className="item-main"><div className="item-title">{g.message || "Runtime error"}</div><div className="item-meta">{g.path || "unknown path"} · {g.event_count} occurrence(s) · last {new Date(g.last_seen).toLocaleString()}</div></div><span className={`badge ${g.severity === "critical" ? "neg" : g.severity === "warning" ? "accent" : "pri"}`}>{g.severity}</span></div>) : <Empty icon={<CheckCircle2 size={22} />} title="No grouped errors" text="No production error groups were returned for the last 30 days." />}</div></div>}
        ...
        {tab === "finance" && <div className="cards-grid" style={{ gridTemplateColumns: "1.35fr 1fr", alignItems: "start" }}><div className="card"><div className="item-row"><div className="item-main"><div className="item-title">Revenue and profit forecast</div><div className="item-meta">Six-month history plus three-month forward run-rate with deterministic confidence.</div></div><TrendingUp size={16} color="var(--primary)" /></div><div className="table-wrap"><table className="tbl"><thead><tr><th>Month</th><th>Revenue</th><th>Expenses</th><th>Profit</th><th>Forecast</th><th>Confidence</th><th>Pending revenue</th></tr></thead><tbody>{forecasts.map((f) => <tr key={f.month_start}><td className="mono">{fmtDate(f.month_start)}</td><td className="mono">{money(f.revenue)}</td><td className="mono">{money(f.expenses)}</td><td className="mono">{money(f.profit)}</td><td className="mono">{money(f.forecast_revenue)}</td><td className="mono">{f.forecast_confidence ?? "—"}%</td><td className="mono">{money(f.pending_revenue)}</td></tr>)}</tbody></table></div></div><div className="card"><div className="item-title">CEO outlook</div><div className="item-meta" style={{ marginTop: 5 }}>Revenue forecast {money(h.forecast_revenue)} · Profit forecast {money(h.forecast_profit)}</div><div className="calc-box" style={{ marginTop: 14 }}><div className="calc-row"><span>Collections</span><b className="mono">{money(forecasts.at(-1)?.collections)}</b></div><div className="calc-row"><span>Outstanding</span><b className="mono">{money(forecasts.at(-1)?.pending_revenue)}</b></div><div className="calc-row"><span>Profitability</span><b className="mono">{Number(h.profitability || 0).toFixed(1)}%</b></div></div></div></div>}
