...
const LazyChat = React.lazy(() => import("./Chat.jsx"));
const LazyPortalHelpdesk = React.lazy(() => import("./PortalHelpdesk.jsx"));
...
const LazyAPNProfile = React.lazy(() => import("./APNProfile.jsx"));
...
const LazyAccounts = React.lazy(() => import("./modules/finance/Accounts.jsx"));
const LazyWithdrawals = React.lazy(() => import("./modules/finance/Withdrawals.jsx"));
const LazyPlanned = React.lazy(() => import("./modules/finance/Planned.jsx"));
const LazyAPNAgreementReader = React.lazy(() => import("./modules/apn/AgreementReader.jsx"));
  ...
  return <div className="content">
    ...
    <div className="cards-grid" style={{ gridTemplateColumns: "repeat(auto-fill,minmax(300px,1fr))" }}>
      {list.length === 0 ? <div className="card" style={{ gridColumn: "1/-1" }}><Empty icon={<Lightbulb size={22} color="var(--muted)" />} title="No ideas saved" text="Park business ideas and future plans here. Turn any of them into a task with one tap." action={<button className="btn primary" onClick={() => openModal({ type: "concept" })}><Plus size={16} />New idea</button>} /></div>
        : list.map((c) => <div key={c.id} className="card stat" style={{ display: "flex", flexDirection: "column", gap: 10 }}>
...
function Team({ team, me, changeProfile, db, resolveResign, onActivity, onOpenAPN }) {
  ...
  const activeApnCount = (db?.apn_users || []).filter((p) => apnEffectiveStatus(p, APN_INACTIVE_DAYS) === "active").length;
...
function sessionAuditFilter(key) { try { return JSON.parse(sessionStorage.getItem(AUDIT_FILTER_SESSION_KEY) || "{}")[key]; } catch { return undefined; } }
...
function APNAgreementCenter({ db, pid, onRefresh }) {
  ...
  return (
    <div>
      ...
      {reading && <LazyAPNAgreementReader formatDate={fmtDate} doc={reading} simple={!!views[reading.id]} onToggleSimple={(s) => setViews((v) => ({ ...v, [reading.id]: s }))} onClose={() => setReading(null)} footer={(() => { const done = satisfied(reading); const acc = myAccepts.get(reading.id); const acceptedNow = !!acc && acc.version === reading.version; return (
...
function APNAI({ meRow, go, mutate, pid }) {
  ...
  return (
    <div className="apn-ai">
      ...
      {quoteOpen && <React.Suspense fallback={<div className="card" aria-busy="true">Loading quotation…</div>}><LazyAPNQuoteWizard meRow={meRow} go={go} onClose={() => setQuoteOpen(false)} onSave={saveQuote} runtime={{ APN_SERVICES, Field, Modal, QUOTE_BUSINESS_EMAIL, QUOTE_DISCLAIMER, QUOTE_SERVICE_LABEL, QUOTE_SITE_TYPES, QUOTE_STEP_LABELS, QUOTE_TECHS, QUOTE_URGENT_RATE, emitToast, money, round2, shareQuoteVia, downloadQuotePdf, supabase, uid }} /></React.Suspense>}
    </div>
  );
}
...
function APNHome({ db, meRow, stats, snap, pid, go, openModal, mutate, onOpenProfile, profile }) {
  ...
  const zStats = apnZoneStats(db, zone.key, apnLivePartners, (u) => apnEffectiveStatus(u, APN_INACTIVE_DAYS), apnPartnerStats, apnZonePeriodKey, round2);
...
function APNDistrict({ db, meRow, mutate }) {
  ...
  const visible = members.filter((p) => (!query || `${p.name} ${p.email} ${apnIdFor(p)} ${p.mobile}`.toLowerCase().includes(query.toLowerCase())) && (status === "all" || apnEffectiveStatus(p, APN_INACTIVE_DAYS) === status));
...
function APNStateHead({ db, meRow, mutate, patchDb, openModal }) {
  ...
  const revenue = round2(converted.reduce((s, l) => s + (Number(l.revenue) || 0), 0));
  const districtRows = districts.map((district) => { const ps = members.filter((p) => p.district === district); const ls = (db.apn_leads || []).filter((l) => ps.some((p) => p.id === l.partnerId)); const cs = ls.filter((l) => l.status === "Converted"); return { district, partners: ps.length, active: ps.filter((p) => apnEffectiveStatus(p, APN_INACTIVE_DAYS) === "active").length, leads: ls.length, converted: cs.length, revenue: round2(cs.reduce((s, l) => s + (Number(l.revenue) || 0), 0)), head: heads.find((h) => h.district === district)?.name || "Unassigned" }; });
  return <div className="apn-head-cockpit">
    ...
    {focus === "overview" && <div className="apn-head-overview-grid"><div className="apn-rowcard"><div className="lbl"><GaugeCircle size={14} />State performance</div><div className="apn-head-statline"><span>Conversion rate</span><b>{leads.length ? Math.round((converted.length / leads.length) * 100) : 0}%</b></div><div className="apn-head-statline"><span>Active partners</span><b>{members.filter((p) => apnEffectiveStatus(p, APN_INACTIVE_DAYS) === "active").length}</b></div><div className="apn-head-statline"><span>Attention required</span><b>{members.filter((p) => ["inactive", "suspended"].includes(apnEffectiveStatus(p, APN_INACTIVE_DAYS))).length}</b></div></div><div className="apn-rowcard"><div className="lbl"><ShieldCheck size={14} />State Head authority</div><p className="hint-line" style={{ lineHeight: 1.6, margin: "8px 0 0" }}>State-wide oversight is read from the APN hierarchy. You can inspect district and partner performance without bypassing administrator-only financial or lifecycle controls.</p></div></div>}
...
export function APNPortal({ db, profile, session, signOut, isDark, mutate, patchDb = () => {}, reload }) {
  ...
  const eff = meRow.status === "rejected" ? "rejected" : (profile.active === false && profile.status !== "pending") ? "suspended" : apnEffectiveStatus(meRow, APN_INACTIVE_DAYS);
  ...
  const section = () => {
    switch (tab) {
      ...
      case "leads": return <React.Suspense fallback={<div className="content"><div className="card" aria-busy="true">Loading leads…</div></div>}><LazyAPNLeads db={db} meRow={meRow} pid={pid} openModal={setModal} mutate={mutate} runtime={{ ...Icons, apnLeadsOf, APN_SERVICE_LABEL, apnLeadTone, APN_LEAD_REJECTED, money, fmtDate, Empty, UserPlus, Plus, Handshake, AlertTriangle }} /></React.Suspense>;
      case "wallet": return <React.Suspense fallback={<div className="content"><div className="card" aria-busy="true">Loading wallet…</div></div>}> <LazyAPNWallet db={db} pid={pid} stats={stats} snap={finSnap} role={meRow?.data?.role || meRow?.role || "partner"} runtime={{ ...Icons, APNMetric, APNWalletDetailModal: LazyAPNWalletDetailModal, APN_COMM_REVERSED, APN_WITHDRAWAL_TYPES, Empty, apnCommTone, apnCommissionProjectsOf, apnCommsOf, apnPayoutDate, apnProjectSummary, apnRequestAmount, apnRevenueCollectionsOf, apnSnapshotWallet, apnWalletLabel, apnWithdrawalLabel, apnWithdrawalTone, apnWithdrawalWalletFor, fmtDate, fmtDateTime, money }} />;</React.Suspense>;
      case "network": return <React.Suspense fallback={<div className="content"><div className="card" aria-busy="true">Loading network…</div></div>}> <LazyAPNNetwork db={db} meRow={meRow} pid={pid} reload={reload} onOpenWithdrawals={() => go("withdrawals")} refreshTick={snapTick} runtime={{ ...Icons, APNReferralMetric, Avatar, Dashboard, Empty, Modal, fmtDate, fmtDateTime, money, referralCodeFor, referralLinkFor, referralQrFor, referralWalletFor, todayISO, Users, supabase, exportRowsToExcel }} />;</React.Suspense>;
      case "chat": return (
...
// ── load data + live sync while signed in ─────────────────────────────
  useEffect(() => {
    ...
    const scheduleReload = (payload, force = false) => {
      ...
      else if (payload != null) forceFullReload = true;
      // A null payload is an internal queue continuation; it must not promote
      // already-recorded dirty tables into a full workspace reload.
      reloadQueued = true;
      ...
      reloadTimer = setTimeout(async () => {
        ...
        reloadInFlight = reload(tables).catch(() => {}).finally(() => {
          reloadInFlight = null;
          if (reloadQueued) scheduleReload(null, false);
        });
      }, 150);
    };
...
const financeComponentHelpers = useMemo(() => ({ todayISO, supabase, emitToast, money, fmtPeriod, fmtDate, expenseScope, SplitBar, ExpenseSharePanel, Empty, USERS, avatarColor, haptic }), [supabase, emitToast]);
      ...
      case "planned": return <LazyPlanned db={db} mutate={mutate} openModal={openModal} removeItem={removeItem} openIncome={openIncome} canFinance={canFinance} helpers={financeComponentHelpers} />;
      ...
      case "documents": return <React.Suspense fallback={<div className="content"><div className="card" aria-busy="true">Loading documents…</div></div>}> <LazyDocuments db={db} mutate={mutate} openModal={openModal} removeItem={removeItem} isAdmin={isAdmin} me={me} runtime={{ ...Icons, Empty, Field, emitToast, fmtDate, avatarColor, DOC_CATEGORIES, KB_CATEGORIES, Notifications: LazyNotifications, Tasks: LazyTasks }} />;</React.Suspense>;
      case "knowledge": return <React.Suspense fallback={<div className="content"><div className="card" aria-busy="true">Loading knowledge…</div></div>}> <LazyKnowledge db={db} mutate={mutate} openModal={openModal} removeItem={removeItem} isAdmin={isAdmin} runtime={{ ...Icons, Empty, Field, emitToast, fmtDate, avatarColor, DOC_CATEGORIES, KB_CATEGORIES, Notifications: LazyNotifications, Tasks: LazyTasks }} />;</React.Suspense>;
  ...
  <ErrorBoundary>
      <div className={"allbee" + (menuOpen ? " menu-open" : "")} data-theme={isDark ? "dark" : "light"}>
        ...
        {searchOpen && <React.Suspense fallback={null}><LazyGlobalSearch db={db} team={team} profile={profile} role={role} me={me} allowedRoutes={[...allowedRoutes]} go={go} openTask={openTask} openModal={openModal} onClose={() => setSearchOpen(false)} nav={NAV} notifVisibleTo={notifVisibleTo} activityModuleOf={activityModuleOf} /></React.Suspense>}
      </div>
    </ErrorBoundary>
  );
}
