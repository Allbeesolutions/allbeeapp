...
export function APNPortal({ db, profile, session, signOut, isDark, mutate, patchDb = () => {}, reload }) {
  ...
  const section = () => {
    switch (tab) {
      ...
      case "profile": return <React.Suspense fallback={<div className="content"><div className="card" aria-busy="true">Loading profile…</div></div>}><LazyAPNProfile db={db} meRow={meRow} stats={stats} snap={finSnap} profile={profile} sessionEmail={session?.user?.email} mutate={mutate} onSignOut={signOut} reload={reload} isHead={isHead} go={go} runtime={{ apnSnapshotWallet, apnSnapshotRate, apnGovernedLimit, useState, useRef, useEffect, apnAvatarUrl, supabase, uploadAttachment, Field, APNMetric, money, TrendingUp, Coins, Award, ShieldHalf, ShieldCheck, apnCalculatedGovernedExplanation, Avatar, Upload, Check, apnIdFor, APNBankDetails, LogOut }} /></React.Suspense>;
      default: return null;
    }
  };
