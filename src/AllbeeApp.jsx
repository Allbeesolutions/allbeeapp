...
export function APNPortal({ db, profile, session, signOut, isDark, mutate, patchDb = () => {}, reload }) {
  ...
  const section = () => {
    switch (tab) {
      ...
      case "district": return isHead ? <APNDistrict db={db} meRow={meRow} mutate={mutate} APN_INACTIVE_DAYS={APN_INACTIVE_DAYS} APNMetric={APNMetric} Empty={Empty} APNHeadPartnerCard={APNHeadPartnerCard} Avatar={Avatar} money={money} round2={round2} /> : isStateHead ? <APNStateHead db={db} meRow={meRow} mutate={mutate} patchDb={patchDb} openModal={setModal} /> : <APNHome db={db} meRow={meRow} stats={stats} snap={finSnap} pid={pid} go={go} openModal={setModal} mutate={mutate} profile={profile} onOpenProfile={() => go("profile")} />;
